...
def Statistics(i_traj, f_traj, diabatic):
    ...
    cis_ave = np.zeros((ntimes, nstates))
    tra_ave = np.zeros((ntimes, nstates))
    ...
    for it in range(n_traj):
        ...
        for n in range(ntimes):
            ...
            for j in range(nstates):
                ...
                cis_ave[n,j] += rho_diab[j,j].real * cis[n]
                tra_ave[n,j] += rho_diab[j,j].real * (1 - cis[n])
    ...
    cis_ave /= n_traj
    tra_ave /= n_traj
    with open('cis_pop.dat', 'w') as pop:
        for n in range(ntimes):
            pop.write(f'{(time[n,0] / fs2au):8.2f}')
            for j in range(nstates):
                pop.write(f'  {cis_ave[n,j]:7.4f} {tra_ave[n,j]:7.4f}')
            pop.write('\n')
